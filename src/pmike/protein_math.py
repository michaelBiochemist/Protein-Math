...
three_to_one = {
    ...
    "TYR": "Y",
    "SEC": "U",
    "PYL": "O",  # Add rare amino acids if needed
}


def get_geometric_CA_center(df, segments=None, selection_criteria=None):
    "Calculates the geometric center of a protein using the positions of its alpha carbons"
    df_CA = None
    ...
    if type(selection_criteria) == str:
        if selection_criteria == "core-sequence":
            ...
            for chain_id, chain_df in df_CA.groupby("segid"):
                ...
                offset = int(
                    max_resnum * 0.15
                )  # Shave off 15% of residues from either side
                start_resnum = min_resnum + offset
                end_resnum = max_resnum - offset

                core_chain_df = chain_df.loc[
                    (chain_df["resnum"] <= end_resnum)
                    & (chain_df["resnum"] >= start_resnum)
                ]
...
def get_volume(df):
    """
    Gets the volume of a dataframe, based on maximum and minimum x, y, and z coordinates
    """
    maxes = df[["x", "y", "z"]].max()
    mins = df[["x", "y", "z"]].min()
    diffs = maxes - mins
...
def extract_sequence(df, chainid):
    ...
    filtered_df = (
        df[(df["segid"] == chainid) & (df["atname"] == "CA")]
        .sort_values("resnum")
        .groupby("resnum")
        .first()
        .reset_index()[["resnum", "resname"]]
    )
    ...
    is_contiguous = len(residue_numbers) == (
        residue_numbers[-1] - residue_numbers[0] + 1
    )
...
def get_alignments(
    ...
    for index, map_row in map_df.iterrows():
        ...
        alignment_list.append(
            {"seq1": aligned_seq1, "seq2": aligned_seq2, "score": normalized_score}
        )
    return alignment_list


def get_alignment_df(
    df1,
    df2,
    map_df,
    mode="global",
    ...
    for index, map_row in map_df.iterrows():
        ...
        for region1, region2 in zip(aligned_seq1, aligned_seq2):
            for i, j in zip(
                range(region1[0], region1[1]), range(region2[0], region2[1])
            ):
                rows.append(
                    {
                        "segid1": map_row["R0"],
...
def different_sequence_RMSD(
    df1, df2, mapper=None, alignment_params=None, center_selection_criteria=None
):
    """
    Get RMSD when sequences are not identical.
    First maps chains from structures to each other by sequence similarity.
    Does a kabsch alignment on the structures using as reference only the parts of the structures with identical sequences, and restricts RMSD calculations to those sections.
    """
    #   #print('df1:\n',df1.head())
    #   #print('df1:\n',df1.segid.unique())
    #   #print('df2:\n',df2.head())
    ...
    if alignment_params == None:
        ...
        alignment_df = get_alignment_df(
            df1,
            df2,
            map_df,
            mode=alignment_params["mode"],
            match_score=alignment_params["match_score"],
            mismatch_score=alignment_params["mismatch_score"],
            gap_open=alignment_params["gap_open"],
            gap_extend=alignment_params["gap_extend"],
        )

    # alignment_df.to_parquet('alignment_df.parquet')

    df1_filtered = pd.merge(
        df1,
        alignment_df,
        how="left",
        left_on=["segid", "resnum"],
        right_on=["segid1", "index1"],
        suffixes=["", "_r"],
    )
    ...
    df1_filtered = df1_filtered.drop(
        columns=["index2", "segid1", "segid2"]
    ).reset_index(drop=True)
    df2_filtered = df2_filtered.drop(columns=["segid1", "segid2"]).reset_index(
        drop=True
    )

    center_by_CA(
        df1_filtered,
        segments=map_df["R0"].unique(),
        selection_criteria=center_selection_criteria,
    )
    center_by_CA(
        df2_filtered,
        segments=map_df["R1"].unique(),
        selection_criteria=center_selection_criteria,
    )
...
def same_sequence_RMSD(df1, df2, mapper=None):
    return kabsch_align(df1, df2, map_df)
...
def merge_CA_dataframes(df0, df1, map_df):
    ...
    if "alignment_index" in df1_CA.keys():
        df2 = pd.merge(
            df_mid,
            df1_CA,
            left_on=["alignment_index", "R1"],
            right_on=["alignment_index", "segid"],
            how="inner",
            suffixes=["_l", "_r"],
        )
    else:
...
def get_CA_RMSD(df0, df1, map_df=None, whitelist_region=None, debuggy=False):
    """
    Calculates the RMSD of two protein structures using their respective alpha carbons.
    Use kabsch align instead of this function.
    ...
    grouped_rmsd = df2.groupby(["R0", "R1"]).apply(
        lambda group: np.sqrt(
            np.mean(np.sum(np.square(group[["dx", "dy", "dz"]]), axis=1))
        )
    )
    ...
    try:
        ...
        map_df["RMSD (N)"] = (
            normalized_rmsd  # Update the map_df with individual RMSD values
        )
    except:
...
def kabsch_align(template_struct, modify_struct, map_df=None):
    """
    Uses the kabsch algorithm to align two dataframes.
    """
    if map_df is None:
        map_df = relational(map_chains(template_struct, modify_struct))
    ...
    final_rmsd_struct = get_CA_RMSD(
        template_struct, modify_struct, map_df, debuggy=False
    )
    return final_rmsd_struct


def pairwise_rmsd(pairwise_tuple):
    """Helper function to compute Pairwise RMSD for parallel execution."""
    # print(pairwise_tuple)
    df0, df1, label0, label1 = pairwise_tuple
    rmsd = kabsch_align(df0, df1)[0]
    return (rmsd, label0, label1)


def compute_rmsd_matrix_parallel(structures, labels, num_jobs=-1):
    """Computes the Pairwise RMSD matrix using parallel processing."""
    num_structures = len(structures)
    rmsd_matrix = pd.DataFrame(
        np.zeros((num_structures, num_structures)), index=labels, columns=labels
    )
    total_computations = num_structures**2 / 2
    ...
    for i in range(num_structures):
        for j in range(i):
            args_list.append((structures[i], structures[j], labels[i], labels[j]))

    count = 0
    with futures.ThreadPoolExecutor(max_workers=8) as executor:
        for result in executor.map(pairwise_rmsd, args_list):
            count += 1
            if count % 1000 == 0:
                print(
                    f"finished job {count} / current percentage: {100*count/total_computations:.1f}"
                )
            rmsd_matrix.loc[result[1], result[2]] = result[0]
            rmsd_matrix.loc[result[2], result[1]] = result[0]
...
def calculate_chirality_proline(residue_df):
    ...
    Assumes the resiude_df passed in as it's argument corresponds only to a single proline residue.
    """
    ...
    if len(HA) == 0:
        ...
        print(
            f'cannot calculate proline {residue_df["resnum"].min()} chirality as alpha H is missing'
        )
        return None
    ...
    N_CA, CA_C, CA_HA = map(
        lambda v: v.flatten() if len(v.shape) > 1 else v, [N_CA, CA_C, CA_HA]
    )
...
def calculate_residue_chirality(residue_df):
    """
    Calculates the chirality of a single amino acid residue using vector arithmetic. Assumes the dataframe passed in as argument is of a single amino acid.
    """
    ...
    if chirality > 0:
        ...
        print(
            f'error calculating residue {residue_df["resname"].min()} {residue_df["resnum"].min()} chirality'
        )
        return None
...
def toggle_residue_chirality(residue_df):
    ...
    projections = (
        np.dot(vec_to_atoms, plane_normal)[:, np.newaxis] * plane_normal
    )  # Projections onto the plane normal
...
def get_chirality(df):
    """
    Gets the chirality of residues across an entire protein dataframe and applies the results back to the original dataframe.
    """
    chirality_series = (
...
def assign_histidine_charge(df, cutoff=4.0):
    ...
    charged = df.loc[
        (df["resname"].isin(["ASP", "GLU", "ARG", "LYS"]))
        & (df["atname"].isin(["OD1", "OD2", "OE1", "OE2", "NH1", "NH2", "NE", "NZ"])),
        columns,
    ]  # Isolate charged residues
    ...
    merged["dist"] = np.linalg.norm(
        merged[["x", "y", "z"]].values - merged[["x_r", "y_r", "z_r"]].values, axis=1
    )
    merged_filter = merged.loc[
        merged["dist"] <= cutoff,
        ["atnum", "resnum", "chainid", "atname", "dist", "interaction"],
    ]
    ...
    histidines.loc[:, "resname"] = histidines.apply(
        lambda row: protonation_map.get((row["resnum"], row["chainid"]), "HSD"), axis=1
    )
    ...
    df_merge = pd.merge(
        df, unique_histidines, how="left", on=["resnum", "chainid"], suffixes=["", "_r"]
    )
    df_merge.loc[df_merge["resname"] == "HIS", "resname"] = df_merge.loc[
        df_merge["resname"] == "HIS", "resname_r"
    ]
